...
- 执行类型检查命令：`bun run type-check`（在项目根目录）或 `npx tsgo --noEmit`（在packages/react-type-doc目录）
...
- `README.md` 中的版本号badge（搜索 `badge/npm/v/react-type-doc` 或 `img.shields.io/npm/v/react-type-doc` 并更新版本号）
...
```markdown
[x.x.x]: https://github.com/yourusername/react-type-doc/compare/v上个版本...vx.x.x
```
...
- `prepublishOnly` 钩子会自动执行 `bun run build` 进行构建
...
- [完整更新日志](https://github.com/yourusername/react-type-doc/blob/main/CHANGELOG.md)
- [文档](https://github.com/yourusername/react-type-doc#readme)
```
...
✅ 下一步操作：
1. 在GitHub创建Release: https://github.com/yourusername/react-type-doc/releases/new?tag=v{版本号}
